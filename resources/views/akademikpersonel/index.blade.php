...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Akademik Personel</li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Akademik Personel</h6>
                    <div class="table-responsive">
                        <table id="dataTableExample" class="table">
                            ...
                            @foreach($akademikPersoneller as $personel)
                                <tr>
                                    <td>{{ $personel->id }}</td>
                                    <td>{{ $personel->personel_isim_soyisim }}</td>
                                    <td>{{ $personel->personel_telefon }}</td>
                                    <td>{{ $personel->personel_email }}</td>
                                    <td>{{ $personel->getUnvan('tr') }}</td>
                                    <td>{{ $personel->getUnvan('en') }}</td>
                                    <td>
                                        <a href="{{ route('akademik-personel-duzenle', $personel->id) }}" class="btn btn-primary">Düzenle</a>
                                    </td>
                                    <td>
